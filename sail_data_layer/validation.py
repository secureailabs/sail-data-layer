...
def validate(
    ...
    data_set_format = DatasetFormatEnum.parse_str(dict_header["dataset_packaging_format"])
    if data_set_format == DatasetFormatEnum.csvv1:
        serializer = Csvv1DatasetSerializer("")
        data_set = serializer.read_dataset_for_path(path_dir_dataset)
        return data_set.validate()
    elif data_set_format == DatasetFormatEnum.fhirv1:
...
def validate_data_content_zip(
    ...
    data_model_jsondict: dict,
    data_set_format: DatasetFormatEnum = DatasetFormatEnum.csvv1,
) -> Tuple[bool, List[str]]:
    if data_set_format == DatasetFormatEnum.csvv1:
        serializer = Csvv1DatasetSerializer("")
